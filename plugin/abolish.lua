...
local function mixedcase(word)
  return vim.fn.substitute(camelcase(word), "^.", [[\u&]], "")
end
...
local function create_dictionary(lhs, rhs, opts)
  ...
  for lhs, rhs in pairs(expanded) do
    if case then
      dict[mixedcase(lhs)] = mixedcase(rhs)
...
local subvert_dispatcher = function(opts)
...
local subvert_preview = function(opts, preview_ns)
  highlights = {}
...
vim.api.nvim_create_user_command("S", subvert_dispatcher, {
  bang = true,
  bar = true,
  nargs = "+",
  complete = complete,
...
vim.api.nvim_create_user_command("Subvert", subvert_dispatcher, {
  bang = true,
  bar = true,
  nargs = "+",
  complete = complete,
