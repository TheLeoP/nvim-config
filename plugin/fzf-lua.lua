...
local fzf_path = vim.fn.stdpath "data" .. "/site/pack/core/opt/fzf/bin"
fzf_path = vim.fs.normalize(fzf_path)
vim.env.PATH = vim.env.PATH .. separator .. fzf_path

vim.pack.add {
  -- NOTE: remove build autocmd from init.lua when removing plugin
...
keymap.set("n", "<leader>fl", function()
  fzf.files { prompt = "< Fd plugins >", cwd = vim.fn.stdpath "data" .. "/site/pack/core/opt" }
end, { desc = "Fuzzy search files in plugins dir" })
keymap.set("n", "<leader>fL", function()
  fzf.live_grep { silent = true, prompt = "< Rg plugins >", cwd = vim.fn.stdpath "data" .. "/site/pack/core/opt" }
...
keymap.set("n", "<leader>fp", projects, { desc = "Find projects" })
